...
export default function Home() {
  ...
  const [loadingLegalAcceptance, setLoadingLegalAcceptance] = useState(false);
  const [acceptPrivacy, setAcceptPrivacy] = useState(false);
  const [acceptTerms, setAcceptTerms] = useState(false);
  const [acceptSignatures, setAcceptSignatures] = useState(false);
  ...
  useEffect(() => {
    if (!session?.user?.id) {
      ...
      setLoadingLegalAcceptance(false);
      return;
    }
  ...
  async function loadLegalAcceptance() {
    ...
    setLoadingLegalAcceptance(true);

    try {
      const localAccepted =
        typeof window !== "undefined" &&
        window.localStorage.getItem(`fe-service-legal-${session.user.id}`) === "accepted";
      ...
      if (error) {
        console.error("Rechtliche Zustimmung konnte nicht geladen werden:", error.message);
        setLegalAccepted(false);
        setLoadingLegalAcceptance(false);
        return;
      }
  ...
  async function acceptLegalTerms() {
    ...
    if (!acceptPrivacy || !acceptTerms || !acceptSignatures) {
      alert("Bitte alle Pflichtfelder akzeptieren, um die FE-Service Plattform zu nutzen.");
      return;
    }
    ...
    try {
      ...
      if (typeof window !== "undefined") {
        window.localStorage.setItem(`fe-service-legal-${session.user.id}`, "accepted");
      }

      setLegalAccepted(true);
    } catch (error) {
      console.error("Zustimmung Fallback aktiv:", error);
      ...
      <main className="min-h-screen bg-[#07130d] px-5 py-8 text-white">
        <div className="mx-auto max-w-5xl rounded-[36px] border border-green-500/20 bg-[#0f1d15] p-6 shadow-2xl shadow-black/40 md:p-8">
          <div className="flex flex-col gap-5 md:flex-row md:items-center">
            ...
            <div>
              <p className="text-sm font-black uppercase tracking-[0.28em] text-green-400">FE-SERVICE</p>
              <h1 className="mt-2 text-3xl font-black md:text-5xl">Zustimmung erforderlich</h1>
              <p className="mt-3 max-w-3xl text-sm font-semibold leading-7 text-slate-300 md:text-base">
                Vor Nutzung der Plattform müssen Datenschutz, Nutzungsbedingungen sowie digitale Dokumentation und Signaturen akzeptiert werden.
              </p>
            </div>
          </div>
          ...
          <div className="mt-8 grid gap-5 lg:grid-cols-3">
            <section className="rounded-[28px] border border-green-500/15 bg-[#13241a] p-5">
              ...
              <p className="mt-3 text-sm font-semibold leading-7 text-slate-300">
                Die FE-Service Plattform verarbeitet Kundendaten, Kontaktdaten, Gerätedaten, Tickets, Dokumente, Serviceberichte und Prüfprotokolle zur Durchführung von Service-, Wartungs- und Prüfleistungen.
              </p>
            </section>

            <section className="rounded-[28px] border border-green-500/15 bg-[#13241a] p-5">
              <h2 className="text-xl font-black text-green-400">Nutzungsbedingungen</h2>
              <p className="mt-3 text-sm font-semibold leading-7 text-slate-300">
                Die Plattform darf nur für berechtigte interne und kundenbezogene Serviceprozesse genutzt werden. Manipulationen, unberechtigter Zugriff oder missbräuchliche Nutzung sind untersagt.
              </p>
            </section>

            <section className="rounded-[28px] border border-green-500/15 bg-[#13241a] p-5">
              <h2 className="text-xl font-black text-green-400">Digitale Dokumentation</h2>
              <p className="mt-3 text-sm font-semibold leading-7 text-slate-300">
                Digitale Prüfprotokolle, PDF-Dokumente und elektronische Signaturen werden zur Nachweisführung gespeichert und archiviert.
              </p>
            </section>
          </div>
          ...
          <div className="mt-8 space-y-4 rounded-[30px] border border-green-500/20 bg-[#13241a] p-5">
            <label className="flex items-start gap-4 rounded-2xl bg-[#0f1d15] p-4">
              <input type="checkbox" checked={acceptPrivacy} onChange={(event) => setAcceptPrivacy(event.target.checked)} className="mt-1 h-6 w-6 accent-green-500" />
              <span className="text-sm font-semibold leading-7 text-slate-200 md:text-base">
                Ich akzeptiere die Datenschutzerklärung und stimme der Verarbeitung personenbezogener Daten im Rahmen der FE-Service Plattform zu.
              </span>
            </label>

            <label className="flex items-start gap-4 rounded-2xl bg-[#0f1d15] p-4">
              <input type="checkbox" checked={acceptTerms} onChange={(event) => setAcceptTerms(event.target.checked)} className="mt-1 h-6 w-6 accent-green-500" />
              <span className="text-sm font-semibold leading-7 text-slate-200 md:text-base">
            ...
            <label className="flex items-start gap-4 rounded-2xl bg-[#0f1d15] p-4">
              <input type="checkbox" checked={acceptSignatures} onChange={(event) => setAcceptSignatures(event.target.checked)} className="mt-1 h-6 w-6 accent-green-500" />
              <span className="text-sm font-semibold leading-7 text-slate-200 md:text-base">
                Ich stimme der digitalen Speicherung von Signaturen, Prüfprotokollen, Serviceberichten und Dokumentationen zu.
              </span>
            </label>
          </div>
          ...
          <button
            ...
            className="mt-8 w-full rounded-[28px] bg-green-500 px-8 py-5 text-xl font-black text-black shadow-lg shadow-green-950/30 transition hover:bg-green-400 active:scale-[0.99]"
          >
            Akzeptieren & Plattform starten
          </button>

          <p className="mt-6 text-center text-xs font-semibold leading-6 text-slate-500">
            FE-Service e.K. · Fitness Equipment Service · Digitale Service-, Wartungs- und Dokumentationsplattform. Hinweis: Diese technische Einwilligung ersetzt keine individuelle Rechtsberatung.
          </p>
        </div>
      </main>
    );
